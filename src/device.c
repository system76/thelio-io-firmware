...
void device_init(struct Device * device) {
    pin_set_dir(device->input, 0);
    pin_set(device->input, 1);

    pin_set_dir(device->output, 1);
    pin_set(device->output, 0);

    device->input_last = pin_get(device->input);
...
void device_step(struct Device * device, uint64_t time) {
    // Run input items
    uint8_t input = debounce_step(&device->input_db, time, pin_get(device->input));
    if (input) {
...
void device_destroy(struct Device * device) {
    ...
    pin_set_dir(device->output, 0);
    pin_set(device->output, 0);

    pin_set_dir(device->input, 0);
    pin_set(device->input, 0);
}

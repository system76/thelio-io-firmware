...
uint64_t time_get(void) {
    ...
    return overflows * TIME_OVERFLOW_INTERVAL + ((uint64_t)tcnt_0) * TIME_TCNT_INTERVAL;
}

void time_set(uint64_t time) {
    uint64_t overflows = time / TIME_OVERFLOW_INTERVAL;
    uint8_t tcnt = (uint8_t)((time % TIME_OVERFLOW_INTERVAL) / TIME_TCNT_INTERVAL);

    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
        time_overflows = overflows;
        *(time_tcnt) = tcnt;
    }
}
